...
public class AccountServiceImpl implements AccountService {
	...
	public AccountServiceImpl(AccountInfoRepository accountInfoRepository, CustomerRepository customerRepository,
			BranchRepository branchRepository, AccountRepository accountRepository, ModelMapper modelMapper,
...
//    		 
	...
	@Override
	public Response getByAcNo(Long id) {
		AccountInfo acInfo = accountInfoRepository.findByIdAndIsActiveTrue(id);
		Account account = accountRepository.findByAccountInfoAndIsActiveTrue(acInfo);
		if (account != null) {
			modelMapper.getConfiguration().setPropertyCondition(Conditions.isNotNull());
			AccountDto accountDto = modelMapper.map(account, AccountDto.class);
			if (account != null) {
